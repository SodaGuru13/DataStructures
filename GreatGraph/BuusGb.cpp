...
void graph::bfssp(int x)
{
	...
	while (dSize > 0)
	{
		...
		if (!mark[v - 1])
		{
			...
			for (j = 0; j < i; ++j)
			{
				if (arr[v - 1][j] && !mark[j] && !isIn(d,dSize, nodes[j]))
				{
					End(d, &dSize, nodes[j]);
					cout << "{" << v << "," << nodes[j] << "}; ";
				}
...
void graph::prim(int startN)
{
	...
	for (numU = numU; numU >= 0; numU--)
	{
		...
		for (j = 0; j < numE / 3; j++)
		{
			if ((isIn(visited, numV + 1, edges[j * 3 + 0]) || isIn(visited, numV + 1, edges[j * 3 + 1]))
				&& (isIn(unvisited, numU + 1, edges[j * 3 + 0]) || isIn(unvisited, numU + 1, edges[j * 3 + 1])))
			{
				if (edges[j * 3 + 2] < minE)
				{
					minE = edges[j * 3 + 2];
...
{
	int localNE = numE / 3;
	int* orderedEIBW = new int[localNE];
	int* numOEIBW = 0;
	int* orderedE = new int[localNE];
	int j, l, cost, minW, minI, numKEOI, node1DSI, node2DSI, node2I;
	int** nodeDS;
	int* kruskalsEOI;
	...
	for (j = 0; j < localNE; j++)
	{
		minW = INT_MAX;
		minI = -1;
		for (l = localNE - 1; l > -1; l--)
		{
			if (!orderedE[l] && edges[l * 3 + 2] <= minW)
			{
				minW = edges[l * 3 + 2];
				minI = l;
			}
	...
	for (j = 0; j < localNE; j++)
	{
		cout << setw(4) << edges[orderedEIBW[j] * 3 + 0]
			<< setw(4) << edges[orderedEIBW[j] * 3 + 1]
			<< setw(4) << edges[orderedEIBW[j] * 3 + 2]
			<< endl;
	}

	nodeDS = new int* [localNE];
	for (j = 0; j < localNE; j++)
	{
		nodeDS[j] = new int[localNE];
		nodeDS[j][0] = nodes[j];
		for (l = 1; l < localNE; l++)
		{
			nodeDS[j][l] = -1;
	...
	for (j = 0; j < localNE; j++)
	{
		...
		for (l = 0; l < localNE; l++)
		{
			if (isIn(nodeDS[l], localNE, edges[orderedEIBW[j] * 3 + 0]))
			{
				node1DSI = l;
				break;
			}
		...
		for (l = 0; l < localNE; l++)
		{
			if (isIn(nodeDS[l], localNE, edges[orderedEIBW[j] * 3 + 1]))
			{
				node2DSI = l;
				break;
			}
		...
		else
		{
			...
			for (l = 0; l < localNE; l++)
			{
				if (nodeDS[node2DSI][node2I] == -1)
					break;

				if (nodeDS[node1DSI][l] == -1)
				{
					nodeDS[node1DSI][l] = nodeDS[node2DSI][node2I];
					nodeDS[node2DSI][node2I] = -1;
					node2I++;
				}
			}
			kruskalsEOI[++numKEOI] = orderedEIBW[j];
		}
	}

	cost = 0;
	cout << "Kruskals Edge Order " << endl;
	for (j = 0; j <= numKEOI; j++)
	{
		cout << setw(4) << edges[kruskalsEOI[j] * 3 + 0]
			<< setw(4) << edges[kruskalsEOI[j] * 3 + 1]
			<< setw(4) << edges[kruskalsEOI[j] * 3 + 2]
			<< endl;
		cost += edges[kruskalsEOI[j] * 3 + 2];
	}
